...
import { Sun, Moon, Rocket, Home, Code, Volume2, VolumeX } from 'lucide-react';
  ...
  return (
    <header className="w-full flex items-center justify-between py-4 px-6 bg-white/70 dark:bg-slate-900/60 backdrop-blur-md border-b-4 border-amber-100 dark:border-slate-800 rounded-3xl shadow-sm mb-6 transition-all duration-300">
      ...
      <div className="flex items-center gap-3">
        ...
        {/* Exporter button */}
        <button
          onClick={() => setView(currentView === 'exporter' ? 'home' : 'exporter')}
          className={`flex items-center gap-2 px-4 py-2.5 rounded-full border-2 font-semibold text-sm transition-all duration-300 ${
            currentView === 'exporter'
              ? 'bg-purple-600 border-purple-700 text-white shadow-lg shadow-purple-200 dark:shadow-none'
              : 'bg-slate-100 hover:bg-slate-200 border-slate-300 text-slate-700 dark:bg-slate-800 dark:hover:bg-slate-700 dark:border-slate-700 dark:text-slate-300'
          }`}
          title="Get Blogger Embed Code"
        >
          <Code size={16} />
          <span className="hidden sm:inline">Blogger Export</span>
        </button>
